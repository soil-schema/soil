...
class Field extends CodeElement {
  ...
  renderSwiftMember () {
    var result = [`var ${this.name}: ${swift.convertType(this.type)}`]
    var hasDescription = false
    if (typeof this.description == 'string') {
      result.unshift(...this.description.trim().split('\n').map(line => `/// ${line}`))
      var hasDescription = true
...
class Response extends CodeElement {
  ...
  renderSwift() {
    if (this.context.response == null) { return 'typealias Response = Void' }
...
const swift = {
  ...
  pretty (code, config) {
    ...
    for (var line of lines) {
      ...
      const hasBlockSignature = /^(?:(public|open|private|fileprivate)(?:\(set\))?\s+)*(var|let|struct|class|init|deinit|func|protocol)/.test(line)
